...
function getSplitStatus(totalPercentage: number) {
  if (Math.abs(totalPercentage - 100) < 0.001) {
    return "Complete - 100% allocated";
  }

  if (totalPercentage > 100) {
    return `${(totalPercentage - 100).toFixed(2)}% overallocated`;
  }

  return `${(100 - totalPercentage).toFixed(2)}% remaining`;
}

function getPdfFileName(songTitle: string) {
  const safeTitle = (songTitle.trim() || "music-split-summary")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");

  return `${safeTitle}-split-summary`;
}
...
export function MusicSplitCalculator() {
  ...
  const splitStatus = getSplitStatus(totalPercentage);
  ...
  function downloadPdfSummary() {
    ...
    document.title = getPdfFileName(songTitle);
    window.print();
  ...
  return (
    <>
      <section
        ...
        className="no-print mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.9fr_1.1fr]"
      >
        <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5 shadow-glow backdrop-blur-xl sm:p-6">
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button className="btn-secondary" type="button" onClick={downloadPdfSummary}>
      ...
      <section className="print-only print-report" aria-label="PDF split summary">
        <header className="print-report-header">
          <div>
            <p className="print-kicker">Music Split Calculator</p>
            <h1>Split Sheet Summary</h1>
            <p className="print-muted">Generated {generatedAt}</p>
          </div>
        ...
        <div className="print-summary-grid">
          <article>
            ...
            <strong>{songTitle.trim() || "Untitled song"}</strong>
          </article>
          <article>
        ...
        <table className="print-table">
          ...
          <tbody>
            {contributors.map((contributor, index) => {
              const payout = (totalRevenue * Number(contributor.percentage || 0)) / 100;

              return (
                <tr key={`print-${contributor.id}`}>
                  <td>{index + 1}</td>
                  <td>{contributor.name.trim() || "Contributor"}</td>
                  <td>{contributor.role.trim() || "Role"}</td>
                  <td>{Number(contributor.percentage || 0).toFixed(2)}%</td>
        ...
        <section className="print-notes">
          <h2>Notes</h2>
          <p>
            Review all names, roles, percentages, and payout amounts before sending payments or sharing this summary.
          </p>
        </section>
